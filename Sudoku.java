...
public class Sudoku {
	...
	private int row_clicked = -1;
	private int col_clicked = -1;
	...
	public boolean check(int digit) {
		return check(digit, row_clicked, col_clicked);
	}
	...
	public int counting(int digit) {
		for(int row=0; row<9; row++)
			for(int col = 0; col<9; col++) {
				if(puzzle_board[row][col] == digit)
					count_num +=1;
			}
		return count_num;
	}
	public boolean delete_num(int digit) {
		if(counting(digit) == 9)
			return true;
		else
			return false;
